...
/**
 * @brief Print eye coordinates in a formatted way
 * 
 * @param results detection results
 */
void print_eye_coordinates_detailed(std::list<dl::detect::result_t> &results)
{
    int face_id = 0;
    for (std::list<dl::detect::result_t>::iterator prediction = results.begin(); prediction != results.end(); prediction++, face_id++)
    {
        if (prediction->keypoint.size() == 10)
        {
            ...
            // 计算眼间距和中心点
            int eye_distance = std::sqrt(std::pow(right_eye_x - left_eye_x, 2) + std::pow(right_eye_y - left_eye_y, 2));
            int center_x = (left_eye_x + right_eye_x) / 2;
            int center_y = (left_eye_y + right_eye_y) / 2;
            
            ESP_LOGI("eye_coordinates", "=== Face %d ===", face_id);
            ESP_LOGI("eye_coordinates", "Left Eye:  (%3d, %3d)", left_eye_x, left_eye_y);
            ESP_LOGI("eye_coordinates", "Right Eye: (%3d, %3d)", right_eye_x, right_eye_y);
            ESP_LOGI("eye_coordinates", "Eye Center: (%3d, %3d)", center_x, center_y);
            ESP_LOGI("eye_coordinates", "Eye Distance: %d pixels", eye_distance);
        }
    }
}
